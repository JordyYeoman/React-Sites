...
export const getData = (numPoints: number) => {
  const newData = Array.from({ length: numPoints }).map((_, i) => {
    return {
      count: i,
      sensor1: getRandomInt(0, 30),
      sensor2: getRandomInt(0, 30),
      sensor3: getRandomInt(0, 30),
      sensor4: getRandomInt(0, 30),
      sensor5: getRandomInt(0, 30),
    };
  });
  return newData;
};
...
export class StackedBarChart extends Component {
  ...
  createChart() {
    ...
    const zoom = fc.zoom().on("zoom", () => {
      console.log("ZOOM!");
      // @ts-ignore
      const zoomState = zoomTransform(this.myRef.current);
